...
* The `PlayerComponent` receives a `IVideoStreamSource` via constructor Dependency Injection (DI).
...
* The video stream source in use is dynamically selected ad run-time via a command line parameters (see `Run the App`), but it could be selected via a compiled value or a configuration file as well.
...
## Setup
After cloning the repo open a shell and CD into the root folder:
...
Run `npm run app` to render sample data on standard output using the default sample `HD` VideoStreamSource.  
